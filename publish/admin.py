...
def attach_filtered_formfields(admin_class):
    ...
    def formfield_for_foreignkey(self, db_field, request=None, **kwargs):
        ...
        return super_formfield_for_foreignkey(self, db_field, request,
                                              **kwargs)
    ...
    def formfield_for_manytomany(self, db_field, request=None, **kwargs):
        _draft_queryset(db_field, kwargs)
        return super_formfield_for_manytomany(self, db_field, request,
                                              **kwargs)
...
class PublishableAdmin(admin.ModelAdmin):
    ...
    def get_actions(self, request):
        ...
        if 'delete_selected' in actions:
            actions['delete_selected'] = (delete_selected, 'delete_selected',
    ...
    def has_delete_permission(self, request, obj=None):
        ...
        if obj and obj.is_public:
            return False
        return super(PublishableAdmin, self).has_delete_permission(request,
                                                                   obj)
    ...
    def has_publish_permission(self, request, obj=None):
        ...
        return request.user.has_perm(
            opts.app_label + '.' + opts.get_publish_permission())
    ...
    def render_change_form(self, request, context, add=False, change=False,
                           form_url='', obj=None):
        context['has_publish_permission'] = self.has_publish_permission(
            request, obj)

        if obj and obj.publish_state == Publishable.PUBLISH_DELETE:
            adminform, inline_admin_formsets = context['adminform'], context[
                'inline_admin_formsets']
            _make_adminform_readonly(adminform, inline_admin_formsets)

            context.update({
                'title': 'This %s will be deleted' % force_unicode(
                    self.opts.verbose_name),
            })

        return super(PublishableAdmin, self).render_change_form(request,
                                                                context, add,
                                                                change,
                                                                form_url, obj)
...
class PublishableBaseInlineFormSet(BaseInlineFormSet):
    ...
    def save_existing_objects(self, commit=True):
        saved_instances = super(PublishableBaseInlineFormSet,
                                self).save_existing_objects(commit=commit)
...
for admin_class in [PublishableAdmin, PublishableStackedInline,
                    PublishableTabularInline]:
