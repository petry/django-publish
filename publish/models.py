...
#
# but we want this to be a reusable/standalone app and have a few different
# needs
...
class PublishableQuerySet(QuerySet):
    ...
    def delete(self, mark_for_deletion=True):
        '''
        override delete so that we call delete on each object separately,
        as delete needs to set some flags etc
        '''
        for p in self:
...
class PublishableBase(ModelBase):
    def __new__(cls, name, bases, attrs):
        new_class = super(PublishableBase, cls).__new__(cls, name, bases,
                                                        attrs)
        # insert an extra permission in for "Can publish" as well as a
        # "method" to find name of publish_permission for this object
...
class Publishable(models.Model):
    ...
    PUBLISH_CHOICES = (
        (PUBLISH_DEFAULT, 'Published'), (PUBLISH_CHANGED, 'Changed'),
        (PUBLISH_DELETE, 'To be deleted'))
    ...
    publish_state = models.IntegerField('Publication status', editable=False,
                                        db_index=True, choices=PUBLISH_CHOICES,
        ...
        if not self.is_public and mark_changed:
            if self.publish_state == Publishable.PUBLISH_DELETE:
                raise PublishException(
                    "Attempting to save model marked for deletion")
    ...
    def publish(self, dry_run=False, all_published=None, parent=None):
        ...
        if self.publish_state == Publishable.PUBLISH_DELETE:
            self.publish_deletions(dry_run=dry_run,
                                   all_published=all_published,
                                   parent=parent)
            return None
        else:
            return self.publish_changes(dry_run=dry_run,
                                        all_published=all_published,
                                        parent=parent)
    ...
    def publish_changes(self, dry_run=False, all_published=None, parent=None):
        '''
        publish changes to the model - basically copy all of it's content to
        another copy in the  database.
        if you set dry_run=True nothing will be written to the database.
        combined with the all_published value one can therefore get
                    ...
                    if issubclass(related, Publishable):
                        if value is not None:
                            value = value._get_public_or_publish(
                                dry_run=dry_run, all_published=all_published,
                                parent=self)
            ...
            if issubclass(obj.model, Publishable):
                ...
                if self.public and not dry_run:
                    if obj.field.rel.multiple:
                        ...
                        deleted_items = getattr(self.public, name).exclude(
                            pk__in=public_ids)
