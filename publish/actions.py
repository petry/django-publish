...
delete_selected.short_description = \
    "Mark %(verbose_name_plural)s for deletion"
...
undelete_selected.short_description = \
    "Un-mark %(verbose_name_plural)s for deletion"
...
def _get_publishable_html(admin_site, levels_to_root, value):
    ...
    if has_admin:
        ...
        model_text = '%s (%s)' % (
            model_text, modeladmin.get_publish_status_display(value)
        )
...
def _to_html(admin_site, items):
    ...
    for value in items:
        if isinstance(value, Publishable):
            html_value = _get_publishable_html(admin_site, levels_to_root,
                                               value)
        else:
